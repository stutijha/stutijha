...
const response = await fetch("https://api.github.com/graphql", {
  ...
  body: JSON.stringify({
    query,
    variables: { login: username, from: iso(from), to: iso(to) }
  })
});
...
const days = body.data.user.contributionCalendar.weeks
...
const gridX = days.map((_, i) => `
  <line x1="${x(i)}" y1="${top}" x2="${x(i)}" y2="${top + plotH}" stroke="#183b66" stroke-width="1" stroke-dasharray="2 5"/>
  <text x="${x(i)}" y="${height - 25}" text-anchor="middle" fill="#2f81f7" font-size="11" font-family="Arial, sans-serif">${i + 1}</text>
`).join("");
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="${width / 2}" y="27" text-anchor="middle" fill="#2f81f7" font-size="18" font-weight="700" font-family="Arial, sans-serif">${esc("Stuti's Contribution Graph")}</text>
  ...
  ${dots}
  <text x="${width / 2}" y="${height - 5}" text-anchor="middle" fill="#2f81f7" font-size="12" font-family="Arial, sans-serif">Days</text>
</svg>
`;

fs.mkdirSync("assets", { recursive: true });
fs.writeFileSync("assets/contribution-activity.svg", svg);
console.log(`Generated graph for ${days.length} days; total in range: ${days.reduce((s, d) => s + d.contributionCount, 0)}`);
